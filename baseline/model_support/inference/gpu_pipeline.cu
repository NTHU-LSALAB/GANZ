...
/*
 * Parse the client-tokenized GPT-2 wire form `ids:<id0>,<id1>,...`.
 *
 * Returns -1 when the request is not in pretokenized mode, a positive token
 * count on success, and one EOT token for malformed pretokenized input. The
 * latter keeps TensorRT indexing in bounds while making invalid input
 * deterministic. Rows are already zeroed by the caller.
 */
__host__ __device__ static int gazsi_parse_gpt2_ids(const char *text, uint32_t limit,
						    int64_t *ids, int64_t *mask,
						    int seq_len)
{
	if (limit < 4 || text[0] != 'i' || text[1] != 'd' ||
	    text[2] != 's' || text[3] != ':')
		return -1;
	...
	while (pos < limit && text[pos] != '\0' && count < seq_len) {
		uint64_t value = 0;
		int digits = 0;
		while (pos < limit && text[pos] >= '0' && text[pos] <= '9') {
			value = value * 10ULL + (uint64_t)(text[pos] - '0');
			if (value >= GAZSI_GPT2_VOCAB_SIZE)
				invalid = true;
			pos++;
			digits++;
		}
					     ...
					     int seq_len)
{
	...
	int gpt2_count = gazsi_parse_gpt2_ids(text, limit, ids, mask, seq_len);
	if (gpt2_count >= 0) {
		token_counts[b] = gpt2_count;
		return;
	}
...
/* GPT-2 LM response: one token ID plus its official tokenizer decode text. */
__global__ static void gazsi_next_token_format_kernel(
					struct inference_ring_buffer *ring,
					struct inference_payload_plane *plane,
					const struct gazsi_batch_item *items,
					int batch_size,
					const int64_t *next_token_ids,
					const int32_t *token_counts,
					const uint32_t *decode_offsets,
					uint32_t decode_vocab_size,
					const char *decode_bytes,
					...
					long elapsed_us)
{
	...
	const char *req = pay->request + it.req_offset;
	char *out = pay->response + (size_t)it.record_index * GAZSI_RESPONSE_STRIDE;
	const int64_t token_id = next_token_ids[b];
	const int max = GAZSI_RESPONSE_MAX;
	int n = 0;
	...
	n += dev_put_str(out + n, "\",\"tokens\":", max - n);
	n += dev_put_i64(out + n, (long long)token_counts[b], max - n);
	n += dev_put_str(out + n, ",\"next_token_id\":", max - n);
	n += dev_put_i64(out + n, (long long)token_id, max - n);
	n += dev_put_str(out + n, ",\"next_token\":\"", max - n);

	if (token_id >= 0 && (uint64_t)token_id < decode_vocab_size) {
		uint32_t begin = decode_offsets[token_id];
		uint32_t end = decode_offsets[token_id + 1];
		if (begin <= end && end <= decode_bytes_size) {
			for (uint32_t i = begin; i < end && n < max; i++)
				out[n++] = decode_bytes[i];
		}
	}
...
extern "C" int gazsi_launch_format_next_tokens(
					struct inference_ring_buffer *ring,
					struct inference_payload_plane *plane,
					const struct gazsi_batch_item *items,
					int batch_size,
					...
					void *stream)
{
	if (!ring || !plane || !items || batch_size <= 0 ||
	    !d_next_token_ids || !d_token_counts || !d_decode_offsets ||
	    decode_vocab_size == 0 || !d_decode_bytes || decode_bytes_size == 0)
		return -1;

	gazsi_next_token_format_kernel<<<1, batch_size, 0, (cudaStream_t)stream>>>(
		ring, plane, items, batch_size, d_next_token_ids, d_token_counts,
		d_decode_offsets, decode_vocab_size, d_decode_bytes,
		decode_bytes_size, elapsed_us);

	cudaError_t err = cudaGetLastError();
	if (err != cudaSuccess) {
		fprintf(stderr, "[GPU_PIPE] next-token format launch failed: %s\n",
			cudaGetErrorString(err));
		return -1;
	}
	return 0;
					...
					int64_t *attention_mask, int seq_len)
{
	...
	if (text) {
		int gpt2_count = gazsi_parse_gpt2_ids(text, (uint32_t)strlen(text),
