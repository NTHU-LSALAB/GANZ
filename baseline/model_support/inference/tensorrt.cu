...
struct TensorRT_Context {
    ...
    bool output_is_next_token;
    int embedding_dim;

    // Official GPT-2 token decode metadata. Control data is loaded once on
    // the host, then kept on the GPU; request/result payload never uses it on
    // the CPU.
    uint32_t *d_decode_offsets;
    char *d_decode_bytes;
    uint32_t decode_vocab_size;
    uint32_t decode_bytes_size;

    ~TensorRT_Context() {
        if (d_decode_offsets != nullptr)
            cudaFree(d_decode_offsets);
        if (d_decode_bytes != nullptr)
            cudaFree(d_decode_bytes);
    }
};

struct GPT2DecodeHeader {
    char magic[8];
    uint32_t vocab_size;
    uint32_t bytes_size;
};

static bool load_gpt2_decode_table(TensorRT_Context *model) {
    const char *path = std::getenv("GAZSI_GPT2_DECODE_TABLE");
    if (path == nullptr || path[0] == '\0') {
        std::cerr << "[TensorRT] Error: next_token_id engine requires "
                  << "GAZSI_GPT2_DECODE_TABLE" << std::endl;
        return false;
    }

    std::ifstream file(path, std::ios::binary);
    if (!file.good()) {
        std::cerr << "[TensorRT] Error: cannot read GPT-2 decode table: "
                  << path << std::endl;
        return false;
    }

    GPT2DecodeHeader header = {};
    file.read(reinterpret_cast<char *>(&header), sizeof(header));
    static const char expected_magic[8] = {'G', 'Z', 'G', 'P', 'T', '2', 'D', '1'};
    if (!file || std::memcmp(header.magic, expected_magic, sizeof(expected_magic)) != 0 ||
        header.vocab_size != GAZSI_GPT2_VOCAB_SIZE || header.bytes_size == 0) {
        std::cerr << "[TensorRT] Error: invalid GPT-2 decode table header" << std::endl;
        return false;
    }

    std::vector<uint32_t> offsets((size_t)header.vocab_size + 1);
    std::vector<char> bytes(header.bytes_size);
    file.read(reinterpret_cast<char *>(offsets.data()),
              (std::streamsize)(offsets.size() * sizeof(uint32_t)));
    file.read(bytes.data(), (std::streamsize)bytes.size());
    if (!file || offsets.front() != 0 || offsets.back() != header.bytes_size) {
        std::cerr << "[TensorRT] Error: truncated GPT-2 decode table" << std::endl;
        return false;
    }
    for (size_t i = 1; i < offsets.size(); i++) {
        if (offsets[i] < offsets[i - 1] || offsets[i] > header.bytes_size) {
            std::cerr << "[TensorRT] Error: corrupt GPT-2 decode offsets" << std::endl;
            return false;
        }
    }

    if (cudaMalloc(reinterpret_cast<void **>(&model->d_decode_offsets),
                   offsets.size() * sizeof(uint32_t)) != cudaSuccess ||
        cudaMalloc(reinterpret_cast<void **>(&model->d_decode_bytes),
                   bytes.size()) != cudaSuccess) {
        std::cerr << "[TensorRT] Error: cannot allocate GPT-2 decode table on GPU"
                  << std::endl;
        return false;
    }
    if (cudaMemcpy(model->d_decode_offsets, offsets.data(),
                   offsets.size() * sizeof(uint32_t), cudaMemcpyHostToDevice) != cudaSuccess ||
        cudaMemcpy(model->d_decode_bytes, bytes.data(), bytes.size(),
                   cudaMemcpyHostToDevice) != cudaSuccess) {
        std::cerr << "[TensorRT] Error: cannot upload GPT-2 decode table" << std::endl;
        return false;
    }

    model->decode_vocab_size = header.vocab_size;
    model->decode_bytes_size = header.bytes_size;
    std::cout << "[TensorRT] GPT-2 decode table loaded on GPU (vocab="
              << header.vocab_size << ", bytes=" << header.bytes_size << ")"
              << std::endl;
    return true;
}
...
extern "C" TensorRT_Model_t* load_tensorrt_engine(const char* engine_path) {
    ...
    model->d_decode_offsets = nullptr;
    model->d_decode_bytes = nullptr;
    model->decode_vocab_size = 0;
    model->decode_bytes_size = 0;
    ...
    model->output_is_next_token = false;
    ...
    for (int i = 0; i < num_tensors; i++) {
        ...
        if (mode == nvinfer1::TensorIOMode::kOUTPUT) {
            if (strcmp(name, "next_token_id") == 0) {
                if (model->engine->getTensorDataType(name) != nvinfer1::DataType::kINT64) {
                    std::cerr << "[TensorRT] Error: next_token_id must be INT64" << std::endl;
                    delete model;
                    return nullptr;
                }
                model->output_tensor_name = "next_token_id";
                model->output_is_next_token = true;
                model->output_has_seq_dim = false;
                model->embedding_dim = 1;
            } else if (strcmp(name, "last_hidden_state") == 0) {
            ...
            if (!model->output_is_next_token && shape.nbDims >= 2) {
    ...
    std::cout << "[TensorRT] Model config: output_tensor=" << model->output_tensor_name
              ...
              << ", output_kind=" << (model->output_is_next_token ? "next_token_id" : "embedding")
              << ", embedding_dim=" << model->embedding_dim << std::endl;

    if (model->output_is_next_token && !load_gpt2_decode_table(model)) {
        delete model;
        return nullptr;
    }
...
extern "C" void simple_tokenize_and_infer_with_context(TensorRT_Model_t* model_ptr, int context_id,
    ...
    try {
        ...
        *token_count = tokenize_text(text, buffers.h_input_ids, buffers.h_attention_mask);
        ...
        size_t output_size = model->output_is_next_token
            ? sizeof(int64_t)
...
static void init_batch_buffers() {
    ...
    /*
     * GPUNetIO keeps long-running RX/TX kernels resident while serving.  Put
     * inference on the device's highest-priority stream so its short tokenizer
     * and TensorRT kernels are admitted ahead of the persistent dataplane work.
     * Lower numeric CUDA priorities are more important.
     */
    int least_priority = 0;
    int greatest_priority = 0;
    if (cudaDeviceGetStreamPriorityRange(&least_priority, &greatest_priority) != cudaSuccess) {
        fprintf(stderr, "[BATCH] Failed to query CUDA stream priority range\n");
        return;
    }
    if (cudaStreamCreateWithPriority(&g_batch_buffers.stream,
    ...
    fprintf(stderr,
            "[BATCH] Device-resident batch buffers initialized (max_batch=%d, "
            "stream_priority=%d, range=%d..%d)\n",
            MAX_BATCH_SIZE, greatest_priority, greatest_priority, least_priority);
}
...
extern "C" int batch_infer_device_resident(TensorRT_Model_t* model_ptr,
    ...
    try {
         ...
         */
        static int graph_replay_enabled = -1;
        if (graph_replay_enabled < 0) {
            const char *disable_graph = std::getenv("GAZSI_DISABLE_CUDA_GRAPH");
            graph_replay_enabled = !(disable_graph != nullptr &&
                                     disable_graph[0] != '\0' &&
                                     std::strcmp(disable_graph, "0") != 0);
            fprintf(stderr, "[BATCH] CUDA Graph replay %s\n",
                    graph_replay_enabled ? "enabled" : "disabled; using enqueueV3");
        }

        if (graph_replay_enabled && g_batch_buffers.graphs_ready &&
            g_batch_buffers.graph_exec[batch_size]) {
        ...
        int format_rc;
        if (model->output_is_next_token) {
            format_rc = gazsi_launch_format_next_tokens(
                ring, plane, d_items, batch_size,
                (const int64_t *)g_batch_buffers.d_output,
                g_batch_buffers.d_token_counts,
                model->d_decode_offsets, model->decode_vocab_size,
                model->d_decode_bytes, model->decode_bytes_size,
                elapsed_us, stream);
        } else {
            format_rc = gazsi_launch_format_results(
                ring, plane, d_items, batch_size,
                (const float *)g_batch_buffers.d_output,
                g_batch_buffers.d_token_counts,
                model->embedding_dim, output_stride(model),
                elapsed_us, stream);
        }
        if (format_rc != 0) {
